...
double collision::isInCollision(collision::Rectangle* rectangle, collision::Circle* circle){
    // 円の右端の画面x座標
...
double collision::isInCollision(collision::Circle* circle, collision::Rectangle* rectangle){
    return isInCollision(rectangle, circle);
}

double collision::isInCollision(collision::Circle* circle1, collision::Circle* circle2){
    // 2乗したまま比較
    // 半径の合計
    ...
    if(dist <= radius_sum){
        // 衝突する場合力積の方向を計算
        double dir = atan2f(circle1->center_display_y - circle2->center_display_y, circle1->center_display_x - circle2->center_display_x);
        if(dir < 0){
            return dir + M_PI * 2;
        }else{
            return dir;   
